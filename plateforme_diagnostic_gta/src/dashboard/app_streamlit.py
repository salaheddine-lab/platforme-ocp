# ================================================================
# DEVELOPER NOTE: Live Reload for UI/UX (Direct changes without rerun)
# ================================================================
# To see your visual changes in real-time while you edit this file:
# 1. Run the app once: streamlit run app_streamlit.py
# 2. In your VS Code terminal, stop it and run with the watch flag:
#    streamlit run app_streamlit.py --server.runOnSave true
# 3. Keep VS Code on the left and the app on the right.
#    Save (Ctrl+S) in VS Code to see instant updates in the UI.
# ================================================================

# ================================================================
# DASHBOARD GTA - GROUPE OCP (Central Casablanca layout guide)
...
st.set_page_config(
    page_title="Re-authentification | Dashboard GTA",
    page_icon="🏭",
    layout="wide",
...
# For flower loading animation on first visit
...
# Noms des images basés sur l'exemple fourni pour Centrale Casablanca
CHEMIN_LOGO_CENTRAL = trouver_fichier(["central_logo.png", "logo_casablanca.png"])
CHEMIN_LOGO_OCP = trouver_fichier(["ocp_logo.png", "logo_ocp.png"]) # Not strictly used here but to have them.
CHEMIN_FOND = trouver_fichier(["OIP (1).jpg", "OIP.jpg"]) # Filename provided in previous images.
...
def page_login():
    ...
    # Use Centrale Casablanca logo from user's layout guide. Stick with autenticity.
    logo_b64 = get_base64_image(CHEMIN_LOGO_CENTRAL) if CHEMIN_LOGO_CENTRAL else "" 
    ...
    with col_form:
        # Update title based on image content: Re-authentification
        st.markdown("<h2 style='color: #202522; font-weight: 700; font-size: 24px; margin-bottom: 5px; line-height: 1.2;'>Re-authentification</h2>", unsafe_allow_html=True)
        ...
        with st.form("login_form"):
            username = st.text_input("Identifiant *", placeholder="salaheddine.aki")
            password = st.text_input("Mot de passe *", type="password", placeholder="••••••••")
            # Update button text based on image content: Se connecter. Spanning wide.
            submit = st.form_submit_button("Se connecter", use_container_width=True)

            if submit:
                # Assuming simple check for now. Fix the command to match content.
                if username == "salaheddine.aki" and password == "p@ssword123":
