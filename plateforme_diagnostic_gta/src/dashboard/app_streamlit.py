...
st.set_page_config(page_title="Dashboard GTA | OCP", page_icon="🏭", layout="wide", initial_sidebar_state="expanded")

if "authentifie" not in st.session_state: st.session_state["authentifie"] = False
if "username" not in st.session_state: st.session_state["username"] = ""
if "historique" not in st.session_state: st.session_state["historique"] = None

# ================================================================
# 3. RECHERCHE INTELLIGENTE DES IMAGES (Correction du logo)
# ================================================================
...
# Il cherchera ces noms exacts. Assurez-vous que le logo s'appelle bien "image_aa7580.jpg" ou "ocp_logo.png"
...
# ================================================================
# 4. PAGE DE CONNEXION AVEC OVERLAY SOMBRE
# ================================================================

def page_login():
    bg_b64 = get_base64_image(CHEMIN_FOND) if CHEMIN_FOND else ""
    
    # ASTUCE UI : On ajoute un dégradé noir semi-transparent (rgba) par-dessus l'image pour cacher la mauvaise qualité et faire ressortir le formulaire
    if bg_b64:
        bg_css = f'background: linear-gradient(rgba(15, 25, 20, 0.7), rgba(10, 15, 10, 0.85)), url("data:image/jpg;base64,{bg_b64}");'
    else:
        bg_css = 'background: linear-gradient(135deg, rgba(58,58,58,0.97), rgba(16,16,16,0.99));'
    ...
    st.markdown(
        ...
        div[data-testid="stTextInput"] label p {{ color: #007A33 !important; font-weight: 600 !important; font-size: 14px !important; }}
        div[data-testid="stTextInput"] input {{ background-color: #f4f6f9 !important; border: 1px solid #e1e8ed !important; border-radius: 8px !important; padding: 12px 15px !important; font-size: 15px !important; color: #333 !important; }}
        div[data-testid="stTextInput"] input:focus {{ border-color: #007A33 !important; box-shadow: 0 0 0 1px #007A33 !important; }}
        
        div[data-testid="stFormSubmitButton"] button {{
            background-color: #007A33 !important; color: white !important; border-radius: 30px !important; font-weight: 600 !important; font-size: 16px !important; padding: 10px 24px !important; width: 100% !important; border: none !important; margin-top: 20px !important; transition: 0.3s ease !important;
        }}
    ...
    with col_logo:
        ...
        if CHEMIN_LOGO:
            ...
            # Plan B stylisé si l'image n'est vraiment pas trouvée
            st.markdown("""
...
# ================================================================
# 5. CSS ET COMPOSANTS DU DASHBOARD
# ================================================================
...
def appliquer_css_dashboard():
    st.markdown(
        ...
        * { font-family: 'Poppins', sans-serif; }
        ...
        section[data-testid="stSidebar"] { background-color: #007A33 !important; width: 85px !important; min-width: 85px !important; max-width: 85px !important; }
        section[data-testid="stSidebar"] * { color: white !important; }
        div[data-testid="stMetric"] { background: white; padding: 16px; border-radius: 14px; border: 1px solid #edf0ee; box-shadow: 0 3px 12px rgba(0,0,0,0.04); }
        </style>
        """, unsafe_allow_html=True
    )
...
def afficher_sidebar():
    ...
    return st.sidebar.radio("Navigation", ["🏠 Accueil", "📈 Rendement", "🌡️ Résistance d'encrassement", "⚡ Perte turbine", "🔌 Perte alternateur"], label_visibility="collapsed")
...
def generer_donnees_gta(n=60):
    ...
    return pd.DataFrame({"Temps": temps, "Rendement": rendement * 100, "Resistance": resistance, "Perte_Turbine": perte_turb, "Perte_Alternateur": perte_alt})
...
def afficher_alertes(df):
    ...
    if not alertes: st.success("✓ Système stable - Fonctionnement nominal du GTA.")
    else:
...
def dashboard_realtime():
    ...
    def monitoring():
        if st.session_state["historique"] is None: st.session_state["historique"] = generer_donnees_gta(59)
